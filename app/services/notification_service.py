...
from ..models import User, Message, Application, Job, Worker, Employer
...
class NotificationService:
    ...
    def notify_new_message(sender_id, receiver_id, message_content):
        ...
        try:
            ...
            current_app.logger.info(f'New message notification: from {sender_id} to {receiver_id}')
    ...
    def notify_application_status_change(application_id, new_status):
        ...
        try:
            ...
            current_app.logger.info(f'Application status change notification: application {application_id} to {new_status}')
        except Exception as e:
            current_app.logger.error(f'Error sending application status notification: {e}')
    ...
    def notify_new_application(job_id, application_id):
        ...
        try:
            ...
            current_app.logger.info(f'New application notification: job {job_id}, application {application_id}')
        except Exception as e:
    ...
    def send_in_app_notification(user_id, title, message, notification_type='info'):
        ...
        """
        try:
            socketio.emit('in_app_notification', {
                'title': title,
                'message': message,
            ...
            current_app.logger.info(f'In-app notification sent to user {user_id}: {title}')
        except Exception as e:
