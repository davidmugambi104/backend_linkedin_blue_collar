...
from ..models import Payment, Job, Worker, Employer, Application
...
class PaymentService:
    ...
    @staticmethod
    def get_payment_summary(employer_id=None, worker_id=None, start_date=None, end_date=None):
        """
    ...
    def get_worker_earnings(worker_id, start_date=None, end_date=None):
        ...
        pending_earnings = sum(p.net_amount for p in payments if p.status.value == 'pending')
        ...
        for payment in payments:
            if payment.paid_at:
                ...
                monthly_earnings[month_key] = monthly_earnings.get(month_key, 0) + float(payment.net_amount)
