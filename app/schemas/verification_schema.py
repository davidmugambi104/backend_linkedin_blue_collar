...
class VerificationSchema(Schema):
    ...
    verification_type = fields.Str(required=True, validate=validate.Length(min=2, max=100))
...
class VerificationUpdateSchema(Schema):
    status = fields.Str(required=True, validate=validate.OneOf([status.value for status in VerificationStatus]))
