...
from ..utils.permissions import employer_required, admin_required
...
def create_review():
    ...
    if job.status != JobStatus.COMPLETED:
        return jsonify({'error': 'Reviews can only be submitted for completed jobs'}), 400
...
def get_worker_average_rating(worker_id):
    ...
    return jsonify({
        'worker_id': worker_id,
        'average_rating': float(worker.average_rating) if worker.average_rating else 0.0,
