...
from ..schemas import ApplicationSchema, ApplicationCreateSchema, ApplicationUpdateSchema
from ..utils.permissions import worker_required, employer_required, admin_required
...
def update_application(application_id):
    ...
    if current_user.role.value == 'employer':
        ...
        if 'status' in data and data['status'] not in ['accepted', 'rejected']:
            return jsonify({'error': 'Employers can only set status to accepted or rejected'}), 400

        if data.get('status') == 'accepted':
            from ..models.job import JobStatus
            if job.status != JobStatus.OPEN:
                return jsonify({'error': 'Cannot accept application for a job that is not open'}), 400
            ...
            if existing_accepted and existing_accepted.id != application_id:
                return jsonify({'error': 'Another application has already been accepted for this job'}), 400
        ...
        if 'status' in data and data['status'] != 'withdrawn':
            return jsonify({'error': 'Workers can only withdraw their applications'}), 400
...
def delete_application(application_id):
    ...
    if current_user.role.value == 'worker':
        worker = Worker.query.filter_by(user_id=current_user_id).first()
        if not worker or application.worker_id != worker.id:
            return jsonify({'error': 'You do not have permission to delete this application'}), 403
    elif current_user.role.value == 'admin':
        pass  # Admin can delete any
    else:
        return jsonify({'error': 'You do not have permission to delete applications'}), 403
