...
def create_payment():
    ...
    if job.status != JobStatus.COMPLETED:
        return jsonify({'error': 'Payments can only be created for completed jobs'}), 400
    ...
    if current_user.role.value == 'employer':
        ...
        if not employer or job.employer_id != employer.id:
            return jsonify({'error': 'You can only create payments for jobs you posted'}), 403
...
def get_payment_stats():
    ...
    total_amount = db.session.query(db.func.sum(Payment.amount)).filter(Payment.status == 'paid').scalar() or 0
    total_fees = db.session.query(db.func.sum(Payment.platform_fee)).filter(Payment.status == 'paid').scalar() or 0
    ...
    recent_payments = Payment.query.filter(Payment.created_at >= thirty_days_ago).count()
