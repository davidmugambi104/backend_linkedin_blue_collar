...
from ..models import User, Worker, WorkerSkill, Skill, Job, Application, Review
from ..schemas import WorkerSchema, WorkerUpdateSchema, WorkerSkillSchema
from ..utils.permissions import worker_required, user_is_owner_or_admin
...
def add_worker_skill():
    ...
    existing = WorkerSkill.query.filter_by(worker_id=worker.id, skill_id=data['skill_id']).first()
    if existing:
...
def update_worker_skill(skill_id):
    ...
    worker_skill = WorkerSkill.query.filter_by(worker_id=worker.id, skill_id=skill_id).first_or_404()
...
def get_recommended_jobs():
    ...
    worker_skill_ids = [ws.skill_id for ws in WorkerSkill.query.filter_by(worker_id=worker.id).all()]
