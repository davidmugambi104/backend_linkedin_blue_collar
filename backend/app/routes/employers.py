...
def search_workers():
    ...
    if "skill_id" in filters:
        ...
        worker_ids = [
            ws.worker_id for ws in WorkerSkill.query.filter_by(skill_id=skill_id).all()
        ]
        ...
        for worker in workers:
            if worker.location_lat and worker.location_lng:
                distance = calculate_distance(
                    employer_lat, employer_lng, worker.location_lat, worker.location_lng
                )
                if distance <= radius:
...
def get_employer_stats():
    ...
    applications = (
        Application.query.join(Job).filter(Job.employer_id == employer.id).all()
    )
